...
int entero(char *id) {
	if(!existe(id)) {
		...
		sprintf(buffer2,"Error semántico: identificador %s ya declarado", id);
		errsemtc++; yyerror(buffer2);
		return 1;
	}
}
...
char* gen_inf(char* A, char OP, char* B) {
	char* temp = new_temp(); entero(temp);
...
int id_no_declarado(char* id){
	if(!existe(id)){
		sprintf(buffer2,"Error semántico: identificador %s no declarado", id);
		errsemtc++; yyerror(buffer2);
		return 1;
	}
	else
